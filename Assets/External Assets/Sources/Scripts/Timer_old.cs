...
public class Timer_old : MonoBehaviour {
	...
	public static Timer_old CreateTimer(Slider slider_timer, float time, bool loop, TimerEvent onEndEvent)
	{
		// Создаем игровой объект и добавляем к нему таймер
		GameObject TimerHandel = new GameObject();
		Timer_old timer = TimerHandel.AddComponent<Timer_old> ();
		// Запускаем таймер
	...
	public static Timer_old CreateTimer(float time, TimerEvent onEndEvent)
	{
		// Создаем игровой объект и добавляем к нему таймер
		GameObject TimerHandel = new GameObject();
		Timer_old timer = TimerHandel.AddComponent<Timer_old> ();
